...
// Demo organization ID - in production this comes from the session
...
export default function UsersPage() {
  ...
  const fetchUsers = useCallback(async () => {
    try {
      const res = await fetch(`/api/v1/users?organizationId=${DEMO_ORG_ID}`);
      if (res.ok) {
  ...
  const onSubmit = async (data: InviteUserInput) => {
    ...
    try {
      const res = await fetch('/api/v1/users/invite', {
        ...
        body: JSON.stringify({ ...data, organizationId: DEMO_ORG_ID }),
      });
